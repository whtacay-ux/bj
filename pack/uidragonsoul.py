...
class DragonSoulWindow(ui.ScriptWindow):
	...
	def SetInventoryPage(self, page):
		...
		for i in xrange(1, 6 if (app.ENABLE_DS_GRADE_MYTH) else 5):
			self.inventoryTab[(page + i) % 6 if (app.ENABLE_DS_GRADE_MYTH) else 5].SetUp()
	...
	def __InventoryLocalSlotPosToGlobalSlotPos(self, window_type, local_slot_pos):
		...
		return (self.DSKindIndex * (6 if (app.ENABLE_DS_GRADE_MYTH) else 5) * player.DRAGON_SOUL_PAGE_SIZE) + self.inventoryPageIndex * player.DRAGON_SOUL_PAGE_SIZE + local_slot_pos
