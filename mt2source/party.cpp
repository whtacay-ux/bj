	...
	while (it != m_memberMap.end())
	{
		...
		if (rMember.pCharacter)
		{
			if (rMember.pCharacter->GetDesc())
			{
				...
				rMember.pCharacter->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<��Ƽ> ��Ƽ�� �ػ� �Ǿ����ϴ�."));
			}
			else
			{
...
void CParty::SummonToLeader(DWORD pid)
{
	...
	{
		l->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<��Ƽ> ��ȯ�Ϸ��� ����� ã�� �� �����ϴ�."));
		return;
	}
...
void CParty::ComputeRolePoint(LPCHARACTER ch, BYTE bRole, bool bAdd)
{
	if (!bAdd)
	{
		...
		*/
		//@turkmmo75
		bool cRefresh = false;
		std::vector<BYTE> PartyPointsList { POINT_PARTY_ATTACKER_BONUS, POINT_PARTY_TANKER_BONUS, POINT_PARTY_BUFFER_BONUS, POINT_PARTY_SKILL_MASTER_BONUS, POINT_PARTY_DEFENDER_BONUS, POINT_PARTY_HASTE_BONUS };
		for (auto pPoints : PartyPointsList)
		{
			if (ch->GetPoint(pPoints) != 0)
			{
				ch->PointChange(pPoints, -ch->GetPoint(pPoints));
				cRefresh = true;
			}
		}
		if (cRefresh)
