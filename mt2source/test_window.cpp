...
void GetFindFileList(LPCTSTR pszDirectory, LPCTSTR pszFilter, std::vector <std::string> &vtList)
{
	TCHAR buf[_MAX_PATH];
	...
	for(;hHandle!=INVALID_HANDLE_VALUE;)
	{
		if( ( FindFileData.cFileName[0]!=_T('.') || lstrlen(FindFileData.cFileName)>2) &&
			( 0 != _stricmp( FindFileData.cFileName, "Replay"))) 
		{
			lstrcpy(buf,pszDirectory);
			lstrcat(buf,_T("\\"));
...
void ReadAllMap(const char * c_pszDirName)
{
	struct stat s;
	char buf[512 + 1];
	...
	for (int j = 0; j < n; j++)
	{
		const char * c_pszFileName = vtFileList[j].c_str();
