...
{
	if (victim->IsDead() || victim->IsObserverMode())//@turkmmo53
		return false;
	if (victim->GetRaceNum() >= 30000 && victim->GetRaceNum() <= 30100)//@turkmmo5
		return false;
	...
	if (!battle_is_icedamage(ch, victim))
		return false;
#endif
	if (ch->IsStun() || ch->IsDead() || ch->IsObserverMode())//@tunga053
		return false;
