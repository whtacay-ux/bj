...
int CInputMain::Chat(LPCHARACTER ch, const char* data, size_t uiBytes)
{
	...
	{
#ifdef ENABLE_CHAT
		std::vector<const char*> kingdoms = { "[1]   ","[2]   ","[3]   " };
...
int CInputMain::Messenger(LPCHARACTER ch, const char* c_pData, size_t uiBytes)
{
	...
	case MESSENGER_SUBHEADER_CG_REMOVE:
	{
		...
		MessengerManager::instance().RemoveFromList(char_name, ch->GetName());//@turkmmo1
	}
	return CHARACTER_NAME_MAX_LEN;
...
void CInputMain::PartyInvite(LPCHARACTER ch, const char* c_pData)
{
	...
	if (!pInvitee || !ch->GetDesc() || !pInvitee->GetDesc() || !pInvitee->IsPC() || !ch->IsPC()) //@turkmmo37
	{
...
void CInputMain::PartyInviteAnswer(LPCHARACTER ch, const char* c_pData)
{
	...
	if (!pInviter || !pInviter->IsPC()) //@turkmmo37
...
void CInputMain::Blackjack(LPCHARACTER ch, const char* c_pData)
{
	TPacketCGBlackjack* p = (TPacketCGBlackjack*)c_pData;
	if (p->subheader == BLACKJACK_SUBHEADER_CG_BET)
		CBlackjackManager::instance().Bet(ch, p->llAmount);
	else if (p->subheader == BLACKJACK_SUBHEADER_CG_HIT)
		CBlackjackManager::instance().Hit(ch);
	else if (p->subheader == BLACKJACK_SUBHEADER_CG_STAND)
		CBlackjackManager::instance().Stand(ch);
	else if (p->subheader == BLACKJACK_SUBHEADER_CG_DOUBLE)
		CBlackjackManager::instance().DoubleDown(ch);
	else if (p->subheader == BLACKJACK_SUBHEADER_CG_SPLIT)
		CBlackjackManager::instance().Split(ch);
	else if (p->subheader == BLACKJACK_SUBHEADER_CG_INSURANCE)
		CBlackjackManager::instance().Insurance(ch);
	else if (p->subheader == BLACKJACK_SUBHEADER_CG_SURRENDER)
		CBlackjackManager::instance().Surrender(ch);
	else if (p->subheader == BLACKJACK_SUBHEADER_CG_CLOSE)
...
int CInputMain::Analyze(LPDESC d, BYTE bHeader, const char* c_pData)
{
	...
	break;
#ifdef ITJA_BLACKJACK_SYSTEM
	case HEADER_CG_BLACKJACK:
		Blackjack(ch, c_pData);
		break;
#endif
#ifdef ENABLE_TARGET_INFORMATION_SYSTEM
