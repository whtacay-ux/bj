...
namespace quest
{
	...
	ALUA(pc_warp_local)
	{
		...
		{
			pkChr->ChatPacket( CHAT_TYPE_INFO, LC_TEXT("�ŷ�â,â�� ���� �� ���¿����� �ٸ������� �̵��Ҽ� �����ϴ�" ));
	...
	ALUA(pc_mount)
	{
		...
		if (mount_vnum)
		{
			if (ch->FindAffect(SKILL_MUYEONG))//@turkmmo2
				ch->RemoveAffect(SKILL_MUYEONG);//@turkmmo2
	...
	ALUA(pc_select_vid)
	{
		...
		{
			CQuestManager::instance().GetPC(new_ch->GetPlayerID());
			lua_pushnumber(L, ch ? (DWORD)ch->GetVID() : 0);//@turkmmo16
		}
		else
		{
			lua_pushnumber(L, 0);
	...
	ALUA(pc_change_empire)
	{
		...
		if (ch->GetParty())
		{
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("You can't change your empire while in the party."));
			lua_pushnumber(L, 4);
			return 0;
		}
		//@turkmmo19
...
#define PC_MI0L_ARG3	7*2		// (type, value)*7
	...
	ALUA(pc_open_shop0)
	{
		...
		{
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("�ٸ� �ŷ�â�� �������¿����� �����ŷ��� �Ҽ� �� �����ϴ�."));
			return 0;
		}
