...
void CHARACTER::EncodeInsertPacket(LPENTITY entity)
{
	...
	if (IsPC() == true || m_bCharType == CHAR_TYPE_NPC)
	{
		...
		addPacket.dwLevel = (IsPC() || IsMonster()) ? GetLevel() : 0;
...
void CHARACTER::SetPlayerProto(const TPlayerTable* t)
{
	...
	if (GetGMLevel() > GM_PLAYER || test_server)//@turkmmo50
	{
...
int CHARACTER::GetPoint(BYTE type) const
{
	...
	if (val > max_val)
		sys_err("POINT_ERROR: %s type %d val %d (max: %d)", GetName(), type, val, max_val);//@turkmmo20
		...
		if (!CTargetManager::instance().GetTargetInfo(pkChrCauser->GetPlayerID(), TARGET_TYPE_VID, GetVID()))
		{
			...
			if (GetMyShop())
			{
				...
				if (pkChrCauser == this) // �ڱ�� ����
				{
					...
					if (IsAcceOpen())
					{
						pkChrCauser->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("�ٸ� �ŷ���(â��,��ȯ,����)���� ����� �� �����ϴ�."));
						return;
					}
#endif
				}
...
bool CHARACTER::AcceIsSameGrade(long lGrade)
{
	...
	bool bReturn = (pkItemMaterial[0]->FindApplyValue(APPLY_ACCEDRAIN_RATE) == lGrade ? true : false);
	return bReturn;
}
...
void CHARACTER::GetAcceCombineResult(DWORD& dwItemVnum, DWORD& dwMinAbs, DWORD& dwMaxAbs)
{
	...
	if (m_bAcceCombination)
	{
		...
		{
			long lVal = pkItemMaterial[0]->FindApplyValue(APPLY_ACCEDRAIN_RATE);
			if (lVal == 20)
			{
...
void CHARACTER::AddAcceMaterial(TItemPos tPos, BYTE bPos)
{
...
#endif
	else if ((m_bAcceCombination) && (bPos == 1) && (!AcceIsSameGrade(pkItem->FindApplyValue(APPLY_ACCEDRAIN_RATE))))
	{
	...
	sPacket.dwPrice = GetAcceCombinePrice(pkItem->FindApplyValue(APPLY_ACCEDRAIN_RATE));
	sPacket.bPos = bPos;
	sPacket.tPos = tPos;
...
void CHARACTER::RemoveAcceMaterial(BYTE bPos)
{
	...
	if (bPos == 1)
	{
		...
		if (pkItemMaterial[0])
			dwPrice = GetAcceCombinePrice(pkItemMaterial[0]->FindApplyValue(APPLY_ACCEDRAIN_RATE));
	}
	else
		ClearAcceMaterials();
...
void CHARACTER::RefineAcceMaterials()
{
	...
	DWORD dwItemVnum, dwMinAbs, dwMaxAbs;
	GetAcceCombineResult(dwItemVnum, dwMinAbs, dwMaxAbs);
	DWORD dwPrice = GetAcceCombinePrice(pkItemMaterial[0]->FindApplyValue(APPLY_ACCEDRAIN_RATE));
