...
void CItem::ModifyPoints(bool bAdd)
{
		...
		if (m_pProto->bType == ITEM_WEAPON || m_pProto->bType == ITEM_ARMOR)
		{
			for (int i = 0; i < ITEM_SOCKET_MAX_NUM; ++i)
			{
				if (IsRealTimeItem() == true)
					continue;
				DWORD dwVnum = GetSocket(i);

				if (dwVnum < ITEM_SOCKET_MAX_NUM - 1)
					continue;
	...
	if (false == CItemVnumHelper::IsRamadanMoonRing(GetVnum())
		|| false == CItemVnumHelper::IsHalloweenCandy(GetVnum())
		|| false == CItemVnumHelper::IsHappinessRing(GetVnum())
		|| false == CItemVnumHelper::IsLovePendant(GetVnum())
	)
	{
...
const char* CItem::GetName()
{
	...
	if (GetProto())
	{
		...
		{
			const DWORD dwSkillVnum = (GetVnum() == 50300 || GetVnum() == ITEM_SKILLFORGET_VNUM) ? GetSocket(0) : 0;
