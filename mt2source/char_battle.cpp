...
bool CHARACTER::Attack(LPCHARACTER pkVictim, BYTE bType)
{
	...
	if (pkVictim->GetMyShop())//@turkmmo5
		return false;
...
{
	return (IS_SET(m_pointsInstant.instant_flag, INSTANT_FLAG_STUN));
...
{
	return (m_pointsInstant.position == POS_DEAD);
...
{
#ifdef ENABLE_GOLD_REWARD_RENEWAL
...
static void GiveExp(LPCHARACTER from, LPCHARACTER to, int iExp)
{
	...
	// set
//@turkmmo55
	int iExpMultipler = GetExpMultipler();
	iExp *= iExpMultipler;
//@turkmmo55
...
struct FuncAggregateMonster
{
	...
	void operator()(LPENTITY ent)
	{
		if (ent->IsType(ENTITY_CHARACTER))
		{
			...
			if (DISTANCE_APPROX(ch->GetX() - m_ch->GetX(), ch->GetY() - m_ch->GetY()) < 10000)//@turkmmo24
				if (ch->CanBeginFight())
					ch->BeginFight(m_ch);
		}
	}
...
void CHARACTER::UpdateAggrPointEx(LPCHARACTER pAttacker, EDamageType type, int dam, CHARACTER::TBattleInfo& info)
{
	...
	if (type != DAMAGE_TYPE_POISON)//@Turkmmo46
