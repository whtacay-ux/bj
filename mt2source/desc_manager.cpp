...
#include <boost/algorithm/string.hpp>//@turkmmo52
struct name_with_desc_func
{
	...
	bool operator () (LPDESC d)
	{
		...
		return false;
		*/
		return d->GetCharacter() && boost::algorithm::to_lower_copy(std::string(d->GetCharacter()->GetName())) == boost::algorithm::to_lower_copy(std::string(m_name));//@turkmmo52
	}
};
